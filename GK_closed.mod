...
//  SS values:
	...
	Lambda_ss	=	log(1);	
	R_ss	=	log(1/betta);	
	Rk_ss	=	log(exp(RkmR_ss)+exp(R_ss));
	...
	RbmR_ss	=	log(Delta * exp(RkmR_ss));		
	Rb_ss	=	log(exp(RbmR_ss)+exp(R_ss));
	Xi  	=	exp(Rb_ss)-1;	
	ib_ss	=	log(Xi+1);	
	...
	Omega_ss	=	log((1-theta)/(1-theta-theta*betta*exp(RkmR_ss)*(1+Delta*exp(x_ss))*exp(phi_ss)));	
	mu_s_ss	=	log(betta*exp(Omega_ss)*exp(RkmR_ss));	
	mu_b_ss	=	log(betta*exp(Omega_ss)*exp(RbmR_ss));	
	nu_ss	=	log(betta*exp(Omega_ss)*exp(R_ss));	
    N_ss	=	log(omega*(exp(K_ss)+exp(qn_ss)*B)/(1-theta*(exp(RkmR_ss)*exp(phi_ss)+exp(RbmR_ss)*exp(phi_ss)*exp(x_ss)+exp(R_ss))));	
    Ne_ss	=	log(theta*(exp(RkmR_ss)*exp(phi_ss)+exp(RbmR_ss)*exp(phi_ss)*exp(x_ss)+exp(R_ss))*exp(N_ss));	
...
model;
    ...
    ir = zlb*(0)+(1-zlb)*max(irstar, 1/exp(ir_ss) -1);
...
options_.slowc     =   1;
options_.maxit_     =   100000;

simul(periods=500, maxit=100);
